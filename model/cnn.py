...
class CNN(nn.Module):
    def __init__(self, in_channels: int = 1, kernel_size: int = 3, img_size: int = 200, dropout_rate: float = 0.2, radius_scale: int = 100) -> None:
        ...
        self.conv1 = nn.Conv2d(
            in_channels=in_channels, out_channels=32, kernel_size=kernel_size, padding=1)
        self.conv2 = nn.Conv2d(
            in_channels=32, out_channels=64, kernel_size=kernel_size, padding=1)
        self.conv3 = nn.Conv2d(
            in_channels=64, out_channels=128, kernel_size=kernel_size, padding=1)
        ...
        self.linear_1 = nn.Linear(
            in_features=128 * dim_3 * dim_3, out_features=1024)
        self.linear_2 = nn.Linear(in_features=1024, out_features=512)
        self.linear_3 = nn.Linear(in_features=512, out_features=3)
